...
import { PlusCircle, Edit3, Trash2, Link2 as LinkIcon, ExternalLink, AlertTriangle } from 'lucide-react';
...
const QuickLinksPage = () => {
  ...
  const [category, setCategory] = useState(''); // Optional category

  useEffect(() => {
    const storedLinks = localStorage.getItem('projectSahurQuickLinks');
    if (storedLinks) {
  ...
  useEffect(() => {
    localStorage.setItem('projectSahurQuickLinks', JSON.stringify(links));
  }, [links]);
  ...
  const handleDelete = (id) => {
    ...
    toast({ title: "Link Deleted", description: `"${linkToDelete.title}" removed.` });
  };
    ...
    >
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-3">
        <h1 className="text-2xl md:text-3xl font-bold flex items-center">
          <LinkIcon className="mr-3 h-7 w-7 text-primary dark:text-secondary"/>Quick Links
        </h1>
      ...
      <Dialog open={showForm} onOpenChange={(isOpen) => { if(!isOpen) resetForm(); else setShowForm(true);}}>
        <DialogContent className="sm:max-w-[425px] bg-card border-border text-card-foreground">
          <DialogHeader>
            <DialogTitle className="text-xl text-foreground">{currentLink ? "Edit Link" : "Add New Link"}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="grid gap-4 py-4">
            <Input placeholder="Link Title (e.g., Course Portal)" value={title} onChange={e => setTitle(e.target.value)} required />
            ...
            <DialogFooter>
              ...
              <Button type="submit">{currentLink ? "Update" : "Add"} Link</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
      ...
      {sortedLinks.length > 0 ? (
            ...
            <Card key={link.id} className="widget-card">
              <CardHeader className="pb-3">
                <CardTitle className="text-lg text-foreground truncate flex items-center">
                  {link.title}
                  <a href={link.url} target="_blank" rel="noopener noreferrer" className="ml-auto text-primary dark:text-secondary hover:opacity-80">
              ...
              <CardContent className="pt-0">
                <a href={link.url} target="_blank" rel="noopener noreferrer" className="text-sm text-primary dark:text-secondary hover:underline truncate block">
                  {link.url}
                </a>
              </CardContent>
              <CardFooter className="flex justify-end gap-2 pt-2 border-t border-border">
                <Button variant="ghost" size="icon" onClick={() => handleOpenForm(link)} className="text-muted-foreground hover:text-primary dark:hover:text-secondary h-7 w-7"><Edit3 className="h-4 w-4"/></Button>
        ...
        >
          ...
          <p className="text-lg font-semibold mb-1">No links saved yet.</p>
          <p className="text-sm">Add your frequently used websites for quick access.</p>
        </motion.div>
      )}
    </motion.div>
  );
